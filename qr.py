...
oled.text("Make WiFi QR", 0, 10)
oled.text("by WIZnet", 0, 20)
oled.show()
...
sock = socket()
sock.bind(('192.168.1.20', 5000)) #Source IP Address
sock.listen(5)
conn, addr = sock.accept()

while True:
    #userinput = input("\nEnter something: ")
    userinput = conn.recv(2048)
    if userinput == "":
        break
    print(userinput)
    qr.clear()
    qr.add_data(userinput.decode('utf-8'))
    matrix = qr.get_matrix()
    print("version:", qr.version)
    print("len of matrix", len(matrix))
